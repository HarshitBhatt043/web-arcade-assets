...
bkcore.hexgl.ShipControls = function (ctx) {
  ...
  if (
    ctx.controlType == 1 &&
    bkcore.controllers.TouchController.isCompatible()
  ) {
    this.touchController = new bkcore.controllers.TouchController(
      domElement,
      ctx.width / 2,
      function (state, touch, event) {
        if (event.touches.length >= 4) window.location.reload(false);
        ...
        // touch was on the right-hand side of the screen
          ...
          if (event.type === "touchend") self.key.forward = false;
    ...
    ctx.controlType == 3 &&
    bkcore.controllers.GamepadController.isCompatible()
  ) {
    this.gamepadController = new bkcore.controllers.GamepadController(function (
      controller
    ) {
    ...
    function updateInfo() {
      if (!isServerConnected) {
        ...
        leapInfo.innerHTML =
          "Put your hand over the Leap Motion Controller to play.";
        leapInfo.style.display = "block";
      }
    }
    updateInfo();

    var lc = (this.leapController = new Leap.Controller({
  ...
  function onKeyDown(event) {
    switch (event.keyCode) {
      case 38:
        /*up*/ self.key.forward = true;
        break;

      case 40:
        /*down*/ self.key.backward = true;
        break;

      case 37:
        /*left*/ self.key.left = true;
        break;

      case 39:
        /*right*/ self.key.right = true;
        break;

      case 81:
        /*Q*/ self.key.ltrigger = true;
        break;
      case 65:
        /*A*/ self.key.ltrigger = true;
        break;

      case 68:
        /*D*/ self.key.rtrigger = true;
        break;
      case 69:
        /*E*/ self.key.rtrigger = true;
        break;
    }
  }
  ...
  function onKeyUp(event) {
    switch (event.keyCode) {
      case 38:
        /*up*/ self.key.forward = false;
        break;

      case 40:
        /*down*/ self.key.backward = false;
        break;

      case 37:
        /*left*/ self.key.left = false;
        break;

      case 39:
        /*right*/ self.key.right = false;
        break;

      case 81:
        /*Q*/ self.key.ltrigger = false;
        break;
      case 65:
        /*A*/ self.key.ltrigger = false;
        break;

      case 68:
        /*D*/ self.key.rtrigger = false;
        break;
      case 69:
        /*E*/ self.key.rtrigger = false;
        break;
    }
  }
...
bkcore.hexgl.ShipControls.prototype.update = function (dt) {
  ...
  if (this.active) {
    if (this.touchController != null) {
      ...
      angularAmount +=
        this.leapBridge.palmNormal[0] * 2 * this.angularSpeed * dt;
      this.speed +=
        Math.max(0.0, 0.5 + this.leapBridge.palmNormal[2]) *
        3 *
        this.thrust *
        dt;
    } else {
  ...
  if (this.mesh != null) {
    ...
    var gradientDelta =
      (this.gradientTarget - (yawLeap + this.gradient)) * this.gradientLerp;
...
bkcore.hexgl.ShipControls.prototype.teleport = function (pos, quat) {
  ...
  if (this.mesh != null) {
    ...
    // Gradient (Mesh only, no dummy physics impact)
    var gradientDelta =
      (this.gradientTarget - this.gradient) * this.gradientLerp;
    if (Math.abs(gradientDelta) > this.epsilon) this.gradient += gradientDelta;
    if (Math.abs(this.gradient) > this.epsilon) {
      this.gradientAxis.set(1, 0, 0);
      this.mesh.matrix.rotateByAxis(this.gradientAxis, this.gradient);
...
bkcore.hexgl.ShipControls.prototype.collisionCheck = function (dt) {
  ...
  if (collision.r < 255) {
    ...
    var lCol = this.collisionMap.getPixel(
      Math.round(lPos.x),
      Math.round(lPos.z)
    ).r;
    var rCol = this.collisionMap.getPixel(
      Math.round(rPos.x),
      Math.round(rPos.z)
    ).r;
    ...
    if (rCol < 128 && lCol < 128) {
      var fCol = this.collisionMap.getPixel(
        Math.round(pos.x + 2),
        Math.round(pos.z + 2)
      ).r;
      if (fCol < 128) {
...
bkcore.hexgl.ShipControls.prototype.heightCheck = function (dt) {
  ...
  if (nheight < 16777)
    this.gradientTarget =
      -Math.atan2(nheight - height, 5.0) * this.gradientScale;
  ...
  x =
    this.heightMap.pixels.width / 2 + this.tiltVector.x * this.heightPixelRatio;
  z =
    this.heightMap.pixels.height / 2 +
  ...
  if (nheight >= 16777) {
    ...
    nheight =
      this.heightMap.getPixelFBilinear(x, z) / this.heightScale +
      this.heightBias;
  }
...
bkcore.hexgl.ShipControls.prototype.getRealSpeed = function (scale) {
  return Math.round(
    (this.speed + this.boost) * (scale == undefined ? 1 : scale)
  );
};
