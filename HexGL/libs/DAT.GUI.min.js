...
DAT.GUI = function (a) {
  ...
  this.add = function () {
    ...
    e = a[c];
    e == void 0 && a.get && (e = a.get(c));
    if (e == void 0)
      DAT.GUI.error(
        a +
          " either has no property '" +
          c +
          "', or the property is inaccessible."
      );
    else if (((a = typeof e), (e = G[a]), e == void 0))
      DAT.GUI.error("Cannot create controller for data type '" + a + "'");
    else {
      for (var f = [this], g = 0; g < arguments.length; g++)
        f.push(arguments[g]);
      if ((e = F(e, f))) {
        ...
        a != "function" &&
          ...
          (e.setValue(DAT.GUI.savedValues[DAT.GUI.saveIndex]),
          DAT.GUI.saveIndex++);
        A();
        q || (k = h);
        if (!b)
          try {
            if (arguments.callee.caller == window.onload)
              (l = n = k = h), (m.style.height = l + "px");
          } catch (i) {}
        return e;
  ...
  this.reset = function () {
    for (var a = 0, b = DAT.GUI.allControllers.length; a < b; a++)
      DAT.GUI.allControllers[a].reset();
  };
...
DAT.GUI.open = function () {
  DAT.GUI.hidden = !1;
  for (var a in DAT.GUI.allGuis)
    DAT.GUI.allGuis[a].domElement.style.display = "block";
};
DAT.GUI.close = function () {
  DAT.GUI.hidden = !0;
  for (var a in DAT.GUI.allGuis)
    DAT.GUI.allGuis[a].domElement.style.display = "none";
};
...
DAT.GUI.getSaveString = function () {
  ...
  for (b in DAT.GUI.allGuis)
    for (var d = DAT.GUI.allGuis[b].appearanceVars(), c = 0; c < d.length; c++)
      a.push(d[c]);
  for (b in DAT.GUI.allControllers)
    DAT.GUI.allControllers[b].type != "function" &&
      ((d = DAT.GUI.allControllers[b].getValue()),
      DAT.GUI.allControllers[b].type == "number" &&
        (d = DAT.GUI.roundToDecimal(d, 4)),
      a.push(d));
  return a.join(",");
};
...
DAT.GUI.replaceGetVar = function (a, b) {
  for (
    ...
    if (((d = i[f].split("=")), d != void 0 && d[0] == a))
      return c.replace(d[1], b);
...
DAT.GUI.getVarFromURL("saveString") != null &&
...
DAT.GUI.Controller.prototype.setValue = function (a) {
  if (this.object[this.propertyName] != void 0)
    this.object[this.propertyName] = a;
  else {
    var b = {};
    b[this.propertyName] = a;
    this.object.set(b);
  }
...
DAT.GUI.Controller.prototype.getValue = function () {
  var a = this.object[this.propertyName];
  a == void 0 && (a = this.object.get(this.propertyName));
  return a;
};
...
DAT.GUI.Controller.prototype.options = function () {
  ...
  b.addEventListener(
    ...
    function () {
      ...
      a.finishChangeFunction != null &&
        a.finishChangeFunction.call(this, a.getValue());
    },
    !1
  );
...
DAT.GUI.ControllerNumber = function () {
  ...
  g.addEventListener(
    ...
    function (b) {
      b.preventDefault();
      a.setValue(
        a.getValue() + (Math.abs(b.wheelDeltaY) / b.wheelDeltaY) * a.getStep()
      );
      return !1;
    },
    !1
...
DAT.GUI.ControllerString = function () {
  ...
  b.addEventListener(
    ...
    function (c) {
      c.keyCode == 13 &&
        a.finishChangeFunction != null &&
        (a.finishChangeFunction.call(this, a.getValue()), b.blur());
      a.setValue(b.value);
    },
    !1
  );
