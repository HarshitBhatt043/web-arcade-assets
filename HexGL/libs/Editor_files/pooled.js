...
gamecore.Pool = gamecore.Base.extend(
  "gamecore.Pool",
  {
    INITIAL_POOL_SIZE: 1,
    ...
    release: function (pooledObj) {
      ...
      if (pool == undefined)
        throw (
          "Oops, trying to release an object of type " +
          pooledObj.Class.fullName +
  ...
  },
  {
    ...
    startTracing: function () {
      ...
      this.tracing = true;
      if (this.traces) this.traces.clear();
      else this.traces = new gamecore.Hashtable();
    },
    ...
    acquire: function () {
      ...
      if (this.tracing) {
        ...
        while (stack[pos].indexOf("Class.addTo") == 0 && pos > 0) pos--;
        var count = this.traces.get(stack[pos]);
        if (count == null) this.traces.put(stack[pos], { value: 1 });
        else count.value++;
      }
    ...
    getStats: function () {
      var s =
        this.Class.fullName + " stats: " + this.freeList.length + " free.\n";
      ...
      if (this.tracing) {
        ...
        for (var k in traceKeys)
          s +=
            traceKeys[k] + " (" + this.traces.get(traceKeys[k]).value + ")\n";
      }
      return s;
    },
...
gamecore.DualPool = gamecore.Pool.extend(
  "gamecore.DualPool",
  {
    acquire: function (classType) {
      var pool = this.getPool(classType);
  ...
  ///
  {
    ...
    expand: function (howMany) {
      ...
      gamecore.Pool.totalPooled += howMany;
      for (var i = 0; i < howMany; i++) this.freeList.add(new this.classType());
    },
 ...
 */
gamecore.DualPooled = gamecore.Base(
  "gamecore.DualPooled",
  ///
  /// STATICS
  ///
