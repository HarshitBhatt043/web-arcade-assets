  ...
  },
  {
    ...
    bindEvents: function () {
      ...
      $(window).on("resize", function () {
        return self.resize.call(self);
      });
      $(document).bind("keydown", function (e) {
        return self.onKeyDown.call(self, e);
      });
      $("#modules").on("drop", function (e) {
        return self.onDropEvent.call(self, e);
      });
    ...
    addDiagramNode: function (key, module) {
      this.diagram.addNode(
        new pgli.diagram.Node(key, module, 50 + 160 * this.nodeCount++, 50)
      );
    },
    ...
    updateDiagram: function () {
      ...
      for (var i = 0, len = this.project.keys.length; i < len; i++) {
        var object = pgli.lang.Parser.parseModule(
          this.project.files.get(this.project.keys[i])
        );
      ...
      for (var i = 0; i < length; i++) {
        ...
        if (i == 0 && this.project == null) {
          var path = window.prompt(
            "Please prove project's root path (with trailing slash).",
            "../files/"
          );
          trace("#Opening new project from [" + fileName + "]...");
          this.bindProject(
            new pgli.Project(path + fileName, function () {
              self.draw();
