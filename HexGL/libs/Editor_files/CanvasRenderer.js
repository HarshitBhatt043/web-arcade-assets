...
pgli.render.CanvasRenderer = gamecore.Base.extend(
  "CanvasRenderer",
  {
    // static
  ...
  },
  {
    ...
    walkModule: function (module) {
      ...
      if ("params" in module)
        for (p in module.params) this.sset(p, module.params[p]);

      if ("vars" in module) this.actionVars(module.vars);

      if ("fill" in module) this.actionFill(module.fill);

      if ("layers" in module)
        for (var i = 0, len = module.layers.length; i < len; ++i) {
          ...
          if ("use" in layer) {
            if ("repeat" in layer) {
              ...
              try {
                iterator = pgli.lang.Parser.parseRepeat(
                  layer.repeat,
                  this.sget()
                );
                trace(
                ...
                trace(
                  "(!) Syntax error in repeat expression [" + layer.repeat + "]"
                );
                continue;
              }
    ...
    walkLayer: function (layer) {
      ...
      for (k in layer) {
        if (k in static.SCOPE_KEYWORDS) continue;
        else if (k in static.SCOPE_XFORMS) {
          var val = pgli.lang.Parser.parseExpression(
            layer[k],
            this.sget(),
            this.xget()
          );
          if (typeof val == "number") {
            ...
            if (p > 0)
              val =
                (Number(val.substr(O, p)) / 100) *
                this.xget()[static.SCOPE_XFORMS[k]];
            else continue;
          }
          if (k == "x" || k == "y") {
          ...
          this.sset(
            k,
            pgli.lang.Parser.parseExpression(layer[k], this.sget()),
            this.xget()
          );
      }
    ...
    actionVars: function (opts) {
      ...
      for (k in opts) {
        if (k in static.SCOPE_KEYWORDS || k in static.SCOPE_XFORMS) continue;
        else
          this.sset(
            k,
            pgli.lang.Parser.parseExpression(opts[k], this.sget(), this.xget())
          );
      }
    },
    ...
    actionFill: function (opts) {
      ...
      if (opts.type == "image") {
        ...
        if ("repeat" in opts && opts.repeat in static.REPEAT_TYPES) {
          var pattern = this.context.createPattern(
            image,
            static.REPEAT_TYPES[opts.repeat]
          );
        ...
        this.context.fillStyle = pgli.lang.Parser.parseExpression(
          opts.value,
          this.sget(),
          this.xget()
        );
        ...
        this.context.strokeStyle = pgli.lang.Parser.parseExpression(
          opts.value,
          this.sget(),
          this.xget()
        );
    ...
    xpush: function (ratio, x, y, h, w) {
      if (ratio == undefined)
        ...
        else this.xform.push(this.xget().slice(0));
      else this.xform.push([ratio, x, y, h, w]);
