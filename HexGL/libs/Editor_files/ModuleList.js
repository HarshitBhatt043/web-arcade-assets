...
pgli.ui.ModuleList = gamecore.Base.extend(
  "ModuleList",
  {
    // static
  ...
  },
  {
    ...
    draw: function () {
      ...
      if (!this.project.isEmpty())
        for (var i = 0, len = this.project.getModulesCount(); i < len; ++i) {
          key = this.project.getModuleKey(i);
          module = this.project.getModule(key);
          modules += static.tplModuleItem
            .replace("$path", key)
            .replace("$name", key);
        }
    ...
    onModuleClick: function (event) {
      ...
      event.data.object.project
        .getAppInstance()
        .showInEditor($(this).attr("data-path"));
