...
pgli.lang.Parser = gamecore.Base.extend(
  "Parser",
  {
    xStruct: {
      scale: 0,
    ...
    parseExpression: function (string, scope, xform) {
      ...
      if (scope != undefined) {
        string = string.replace(this.patternVar, function (match, varName) {
          ...
          else if (s && varName in scope) return scope[varName];
          else return 0;
        });
      }
      ...
      string = string.replace(
        this.patternMethod,
        function (match, methodName, a, params) {
    ...
    parseRepeat: function (string, scope) {
      ...
      if (items.length != 4) throw "Syntax error in repeat expression";
