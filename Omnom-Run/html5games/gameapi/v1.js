!(function (a, b) {
  const SDK_INTERFACE = {
    ...
    init: function (settings) {
      ...
      this.getDebugLevel() &&
        console.log("[init] SDK_INTERFACE %s...", this.version);
      ...
      Promise.all(files)
        ...
        .finally(function () {
          ...
          (_this.settings.forceMockObject
            ...
            .then(
              function () {
                _this.getDebugLevel() &&
                  console.log(
                    "[SDK_INTERFACE] using mock object..." +
                      (!_this.settings.forceMockObject ? "SKIPPED" : "OK")
                  );
              },
              function () {
                _this.getDebugLevel() &&
                  console.log("[SDK_INTERFACE] using mock object... FAILED");
              }
            )
            .finally(function () {
              SDK_INTERFACE_INIT()
                .then(
                  function () {
                    _this.getDebugLevel() &&
                      console.log("[SDK_INTERFACE] init... OK");
                  },
                  function () {
                    _this.getDebugLevel() &&
                      console.log("[SDK_INTERFACE] init... FAILED");
                  }
                )
                .finally(function () {
                  _this.done();
    ...
    _showAd: function (callback, force, isInitial) {
      ...
      if (this.getAdState(type) !== this.AD_STATES.READY) {
        this.getDebugLevel() &&
          console.log(
            "[famobi] showAd SKIPPED (state: %s)",
            this.getAdState(type)
          );
        doCallback();
        return;
      }
      ...
      setTimeout(function () {
        ...
        SDK_INTERFACE_SHOW_AD(force, isInitial)
          .then(
            function () {
    ...
    _rewardedAd: function (callback) {
      ...
      if (this.getAdState(type) !== this.AD_STATES.READY) {
        this.getDebugLevel() &&
          console.log(
            "[famobi] rewardedAd SKIPPED (state: %s)",
            this.getAdState(type)
          );
        doCallback();
        return;
    ...
    preload: function (type) {
      ...
      SDK_INTERFACE_PRELOAD_AD(type).then(
        ...
        function () {
          _this.getDebugLevel() &&
            console.log("[%s] preloading... FAILED", type);
          _this.setAdState(type, _this.AD_STATES.CLOSED);
          setTimeout(function () {
            _this.setAdState(type, _this.AD_STATES.IDLE);
    ...
    setAdState: function (type, state) {
      this.getDebugLevel() &&
        console.log(
          "[%s] changed state: %s",
          type,
          Object.keys(this.AD_STATES)[state]
        );
    ...
    hasCooledDown: function (type) {
      ...
      const lastAdCalled = Math.round(
        (Date.now() - this.settings[type].last) / 1000
      );
      ...
      if (lastAdCalled >= adTimeInterval) {
        ...
        this.getDebugLevel() &&
          console.log(
            "[%s/cooldown]: %s seconds left",
            type,
            adTimeInterval - lastAdCalled
          );
        return false;
      }
    },
      ...
      if (typeof window.famobi !== "undefined") {
        Object.keys(SDK_INTERFACE_OVERRIDES.famobi).forEach(function (fnc) {
          if (["showAd", "rewardedAd", "hasRewardedAd"].includes(fnc)) {
            _this.getDebugLevel() &&
              console.warn("[overwrite] famobi.%s... SKIPPED", fnc);
          } else {
            _this.getDebugLevel() &&
              console.log("[overwrite] famobi.%s... OK", fnc);
            window.famobi[fnc] = SDK_INTERFACE_OVERRIDES.famobi[fnc];
          }
        });

        window.famobi.showAd = SDK_INTERFACE._showAd.bind(SDK_INTERFACE);
        window.famobi.rewardedAd =
          SDK_INTERFACE._rewardedAd.bind(SDK_INTERFACE);
        window.famobi.hasRewardedAd =
          SDK_INTERFACE._hasRewardedAd.bind(SDK_INTERFACE);
      }
      ...
      let intervalAnalytics = setInterval(function () {
        if (typeof window.famobi_analytics !== "undefined") {
          ...
          Object.keys(SDK_INTERFACE_OVERRIDES.famobi_analytics).forEach(
            function (fnc) {
              _this.getDebugLevel() &&
                console.log("[overwrite] famobi_analytics.%s... OK", fnc);
              window.famobi_analytics[fnc] =
                SDK_INTERFACE_OVERRIDES.famobi_analytics[fnc];
            }
          );
        }
      }, 500);
...
if (typeof window !== "undefined" && !window.famobi) {
  !(function (a, b) {
    var famobi = {
                ...
                function (text) {
                  ...
                  if (typeof SDK_INTERFACE_SETTINGS !== "undefined") {
                    ...
                    if ("features" in SDK_INTERFACE_SETTINGS) {
                      let features = Object.keys(
                        SDK_INTERFACE_SETTINGS.features
                      );
                      for (let i = 0; i < features.length; i++) {
                        this.config.features[features[i]] =
                          SDK_INTERFACE_SETTINGS.features[features[i]];
                      }
                    }
                  }
                  ...
                  if (window.famobi_gameJS && window.famobi_gameJS.length) {
                    if (
                      typeof window.famobi_gameJS[
                        window.famobi_gameJS.length - 1
                      ] == "string"
                    ) {
                  ...
                  window.famobi_multiplayer =
                    this.config.features.multiplayer || false;
                  ...
                  var getGameTitle = function () {
                    ...
                    return toTitleCase(
                      window.famobi_gameID.replaceAll("-", " ")
                    );
                  };
                  ...
                  resolve(
                    window.famobi_gameID === "running-jack" ? false : true
                  );
                }.bind(this)
              );
            }
      ...
      showAd: function (callback, force) {
        ...
        this.log("showing ad...");
        this.ads.show(this.ads.show_initial && this.ads.adcount == 1).then(
          function () {
      ...
      gametranslation: {
        init: function () {
          ...
          if (
            window.famobi.gametranslation
              ...
              .indexOf(window.famobi.gametranslation.curLangString) === -1
          ) {
        ...
        translateString: function (key) {
          return window.famobi.gametranslation.translationData.hasOwnProperty(
            key
          )
        ...
        getNavigatorLocale: function () {
          ...
          if (
            navigator.languages &&
            window.famobi.sizeOf(navigator.languages)
          ) {
        ...
        translateHtml: function ($selector) {
          $selector.find("[data-i18n]").each(function () {
            ...
            this.innerHTML =
              window.famobi.gametranslation.translateString(textkey);
          });
          return $selector;
        },
      },
      ...
      __: function (key) {
        ...
        if (this.config.game_i18n) {
          if (this.config.game_i18n[lang]) {
            this.config.game_i18n[lang] = Object.assign(
              this.config.game_i18n[lang],
              this.config.game_i18n[this.config.aid + "." + lang]
            );
          }
          ...
          if (
            this.config.game_i18n.default &&
            this.config.game_i18n.default[key]
          ) {
      ...
      showSplashScreen: function (pCallback, pShowTill_EVENT, pModal) {
        if (!document.getElementById("famobi_splash")) {
          ...
          readSplashFile("html5games/splash/splash.html", (response) => {
            ...
            if (this.config.preload_image)
              splashScreen.querySelector(".logo").src =
            ...
            if (!pShowTill_EVENT) {
              setTimeout(
                () => this.hideSplashScreen(pCallback),
                this.config.splash_duration
              );
            } else {
              window.addEventListener(pShowTill_EVENT, () =>
                this.hideSplashScreen(pCallback)
              );
            }
          });
          ...
          if (!pShowTill_EVENT) {
            ...
            window.removeEventListener(pShowTill_EVENT, () =>
              this.hideSplashScreen(pCallback)
            );
            window.addEventListener(pShowTill_EVENT, () =>
              this.hideSplashScreen(pCallback)
      ...
      hideSplashScreen: function (pCallback) {
        ...
        if (!this.splashScreen._orgAni)
          this.splashScreen._orgAni = this.splashScreen.style.animation; //This saves the original animation - this is required to be able to rerun the animation the next time the splash is shown
      ...
      getAbsolutePath: function (relativePath) {
        ...
        return (
          absolutePath.substr(0, absolutePath.lastIndexOf("/") + 1) +
          relativePath
        );
      },
      ...
      menu: {
        create: function () {
          ...
          if (
            ...
            for (
              ...
              langButtons += `<li class="${
                lang == currlang ? "fg-lang fg-lang-selected" : "fg-lang"
              }" data-switch-lang="${lang}" style="cursor: pointer;">
        ...
        show: function () {
          ...
          var $fullscreenIcon = faZepto(
            "#fg-overlay .fa-menu-button-fullscreen"
          );
      ...
      moreGamesLink: function () {
        ...
        if ((url = this.config.more_games_url)) {
          ...
          confirm("redirecting to... " + url) &&
            (window.open(url, "") || (window.top.location.href = url)); // handles the link target in production version, depends on user language
        }
      },
      ...
      sessionStorage: {
        setItem: function (key, value) {
          try {
            ...
            window.famobi.config.logging &&
              console.log("[SessionStorage] Skipped because of issues");
          }
        },
      ...
      localStorage: {
        setItem: function (key, value) {
          try {
            ...
            window.famobi.config.logging &&
              console.log("[LocalStorage] Skipped because of issues");
          }
        },
        getItem: function (key) {
          try {
        ...
        update: function (orientation) {
          ...
          if (
            ...
            (window.famobi.config.rotation ||
              (window.famobi.config.rotation !== false &&
                !detection.is.pc &&
                !detection.is.tablet)) &&
            window.screen &&
      ...
      getOrientation: function () {
        var orientationMatch = window.matchMedia(
      ...
      hasFeature: function (feature) {
        ...
        return (
          feature in this.config.features && !!this.config.features[feature]
        );
      },
      getAppLink: function () {
        return "";
      ...
      setVolume: function (vol) {
        ...
        if (this.volume != vol) {
          console.warn(
            "volume: value out of bounds. Now forced to " + this.volume
          );
        }
      ...
      adaptersModule: function () {
        ...
        adaptersPrototype.add = function (section, subsection, callback) {
          if (
            !(section in this.adapters && subsection in this.adapters[section])
          ) {
          ...
          if (typeof adapter != "object" || !Array.isArray(adapter.callbacks)) {
            self.log(
              "adapters.add: broken adapter object for",
              section,
              subsection
            );
            return this;
  ...
  Promise.all([loadScript("sdk_interface.js"), isPageReady])
    .then(function () {
      if (
        typeof SDK_INTERFACE !== "undefined" &&
        SDK_INTERFACE.isSDKInterfaceDone !== "undefined"
      ) {
    ...
    .then(function () {
      Promise.all(
        window.famobi.config.ads && !!window.famobi.config.ads.show_initial
          ? [window.famobi.showInterstitialAd(true, true)]
          : []
      ).then(function () {
        setTimeout(function () {
          (function next() {
            ...
            if (typeof currentScript === "function") {
              if (
                ...
                typeof window.faZepto === "undefined"
              ) {
